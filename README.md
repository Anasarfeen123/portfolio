...
- `ProjectSphere` — three concentric shells, not one: featured projects sit on the middle shell (golden-angle spiral placement, billboarded so each card stays flat/readable as it turns, faded by facing direction so the back clears out of the way naturally), while a dense warm inner "core" and a sparse cool outer shell are blank, non-interactive `THREE.Points` clouds that exist purely to make the composition read as a populated 3D space rather than "N cards in a ring" — different radius/speed/color per shell for real parallax depth. Scroll-linked rotation, same interaction model as the CSS carousel it replaced (no free drag — a second pointer-capturing surface competing with page scroll wasn't worth it given this session's own Lenis-vs-scroll-snap history). Falls back to the original `Revolving3DCarousel` (kept, not deleted) when WebGL is unavailable or motion is reduced. `ProjectCard` grid + `ProjectDetailsModal` for the full catalog on `/projects`, which also gets an opt-in live-iframe preview of a project's actual deployed site when it has a `demo` URL, alongside the static screenshot
